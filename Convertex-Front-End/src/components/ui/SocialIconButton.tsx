...
const ICON_SIZES: Record<SocialButtonSize, string> = {
    sm: "w-4 h-4",
    md: "w-5 h-5",
    lg: "w-6 h-6"
};
...
export function SocialIconButton(props: SocialIconButtonProps) {
    ...
    const activeVariant = variant || (VARIANTS[iconKey] ? iconKey : 'primary');
    const variantStyle = VARIANTS[activeVariant] || VARIANTS.primary;
    ...
    const renderIcon = () => {
        if (customSrc) {
            return (
                <img
                    src={customSrc}
                    alt={alt}
                    className={`${ICON_SIZES[size]} object-contain rounded-full`}
                />
            );
        }
        ...
        switch (icon?.toLowerCase()) {
            case 'github':
                return (
                    <svg className={ICON_SIZES[size]} fill="currentColor" viewBox="0 0 24 24">
    ...
    const {
        ...
        className: _cls,
        ...buttonProps
    } = props as SocialIconButtonAsButton;
